...
/*! /brief Digest info
 */
__declspec(dllexport) typedef struct ph_digest {
    char *id;                   //hash id
...
__declspec(dllexport) typedef struct ph_hash_point {
    ulong64 hash;
    off_t index; /*pos of hash in orig file */
} TxtHashPoint;

__declspec(dllexport) typedef struct ph_match{
...
__declspec(dllexport) DP** ph_read_imagehashes(const char *dirname,int capacity, int &count);
...
**/
__declspec(dllexport) uint8_t* ph_mh_imagehash(const char *filename, int &N, float alpha=2.0f, float lvl = 1.0f);
#endif
 ...
 **/
__declspec(dllexport) double ph_hammingdistance2(uint8_t *hashA, int lenA, uint8_t *hashB, int lenB);
 ...
 **/
__declspec(dllexport) TxtHashPoint* ph_texthash(const char *filename, int *nbpoints);
 ...
 **/
__declspec(dllexport) TxtMatch* ph_compare_text_hashes(TxtHashPoint *hash1, int N1, TxtHashPoint *hash2, int N2, int *nbmatches);
